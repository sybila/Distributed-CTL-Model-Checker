...
interface Job<N: Node, C: Colors<C>>: Message<N, C> {
    val target: N
...
interface JobQueue<N: Node, C: Colors<C>, J: Message<N, C>> {
...
class SingleThreadJobQueue<N: Node, C: Colors<C>, J: Job<N, C>> (
    ...
    override fun start(onTask: (J) -> Unit) {
        worker = thread {
            ...
            while (job is Message.Poison) {
                ...
                onTask(job as J)
                synchronized(this) {    //if queue is empty, we are done
                    if (localTaskQueue.size() == 0) terminator.setDone()
                }
            }
